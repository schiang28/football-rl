...
# Define log directory and experiment name
log_dir = "./wandb_logs"
exp_name = f"mappo_{scenario_name}"
...
logger = WandbLogger(
    ...
    log_dir=log_dir,
    # log_interval=1, # Log every step (optional, default is good)
    # The default log_dir is where the WandB run data will be stored locally
)
...
for tensordict_data in collector:
    ...
    if done.any():
        episode_rewards = tensordict_data.get(("next", agent, "episode_reward"))[done]
        episode_reward_mean = episode_rewards.mean().item()
        
        # 2. Log the metric to WandB
        logger.log_scalar(
            name="episode_reward/mean", 
            value=episode_reward_mean, 
            step=log_iteration
        )
        ...
        # Handle case where no episodes finished in this batch (should be rare with max_steps=100)
        pbar.set_description("episode_reward_mean = N/A (no finished episodes)", refresh=False)
...
gif_path = f"./rollout_videos/mappo_{scenario_name}_rollout.gif"
imageio.mimsave(gif_path, all_frames, fps=30) # Save at 30 frames per second


def rollout_temporarily(env, max_steps, policy):
    # rollout environment once and render temporarily
    with torch.no_grad():
        env.rollout(
            max_steps=max_steps,
            policy=policy,
